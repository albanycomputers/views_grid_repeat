...
if ($layout_inline == 'inline_css') {
  ...
  $css_style = '';
  $css_style = 'style="display:grid;grid-template-columns:repeat(auto----, minmax(' . $css_width_value . ', 1fr)); min-height:' . $css_width_height . ';"';

  // The four extra dashes after 'container' below are a holding place for string replacement.
  $div_class = '';
  $div_class = '<div class="grid-repeat-container-----" ';

  switch ($layout_type) {
    case "fit_grid":
      $css_style = str_replace('auto----','auto-fit',$css_style);
      $div_class = str_replace('-----','-fit',$div_class);
    break;

    case "fill_grid":
      $css_style = str_replace("auto----","auto-fill",$css_style);
      $div_class = str_replace('-----','-fill',$div_class);
    break;
  }
